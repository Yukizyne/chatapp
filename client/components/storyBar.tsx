...
export default function StoryBar({onViewStory} : StoriesBarProps) {
    ...
    const {Stories} = {Stories: dummyStoriesData}
  ...
  return (
   ...
   keyExtractor={(item, i)=> (item._addStory ? "add" : item.user._id ||
     String(i))}
     renderItem={({item})=>{
        if(item._addStory){
        return (
            <TouchableOpacity style={styles.storyItem} 
            onPress={pickAndUpload} disabled={upload}>
                <View style={styles.addCircle}>
                    <Ionicons name={upload ?  "hourglass" : 
