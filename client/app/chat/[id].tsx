import { View, Text, Touchable, TouchableOpacity, KeyboardAvoidingView, Platform, ActivityIndicator, FlatList, TextInput, Alert } from 'react-native'
...
export default function ChatScreen() {
    ...
    const { status } = await ImagePicker.
        ...
        if(status !== 'granted'){
          Alert.alert("Permission needed",
            "Allow photos access to send media")
            return;
        }
  ...
  const typingsEnt = Object.entries(typingUsers).filter(([uid,
    isTyping])=> {
    ...
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.emptyState}>
          <Ionicons name='chatbubbles-outline' size={52}
  ...
  const headerSub = partner!.isOnline ? "online" : partner?.lastSeen ? 
  `Last seen ${formatTime(partner.lastSeen)}` : "offline";
    ...
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
      {/* header */}
        ...
        <View style={styles.headerInfo}>
          ...
          <Text style={[styles.headerSub, partner?.isOnline &&
            {color: Colors.online}
          ]}>{headerSub}</Text>
        </View>
              ...
              <View style={styles.inputBar}>
                ...
                {mediaUri && (
                  <View style={styles.mediaPreview}>
                    <Image source={{uri: mediaUri}} style={
                      styles.mediaThumb
                    }/>
                ...
                <View style={styles.inputRow}>
                  <TouchableOpacity style={styles.attachBtn} onPress={
                    ...
                    activeOpacity={0.85} onPress={send}>
